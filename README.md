...
- **关键脚本**：
  ...
  - 评测：`scripts/evaluation/evaluate_dpo_quality.py`
- **结果**：
...
```bash
# 1) 构造 DPO 数据（需要一个已有的 SFT checkpoint 作为采样模型）
...
# 3) pairwise 生成质量评测
python scripts/evaluation/evaluate_dpo_quality.py \
  --base_model_path ./qwen/Qwen2___5-3B-Instruct \
  --sft_adapter_path ./qwen_finance_sft/final \
  --dpo_adapter_path ./qwen_cflue_dpo/final
```
