...
|------|--------|-------------|----------|----------|
| **SFT** | Fineval val（1,151 题，严格格式） | 67.94%（宽松格式） | **73.76%** | 格式规范化提升 5.8 pp |
| **IPO** | 300 题 pairwise 质量评估 | SFT 生成质量 | **45.0% 胜率**（SFT 仅 23.3%） | 生成质量显著提升 |
...
1. **开放式 SFT 有效**：在不使用单项选择题作为 SFT 数据的情况下，模型在 Fineval 上从基座 67.59% 提升到严格格式下的 **73.76%**。
2. **输出格式对评测分数影响很大**：强制使用“答案：X / 解析：…”格式后，SFT 的 Fineval 分数从 67.94% 提升到 **73.76%**。
3. **SFT 弱项**：投资学（36.84%）、金融工程（50.00%）、宏观经济学（54.84%）、计量经济学（55.56%）等科目仍有较大提升空间。
4. **IPO 显著提升生成质量**：在专业性、推理清晰度、完整性等维度上全面超过 SFT，pairwise 胜率提升到 **45.0%**。
5. **生成质量与选项正确率存在 trade-off**：IPO 的生成质量更好，但 Fineval 选项正确率从 71.3% 下降到 69.7%。
